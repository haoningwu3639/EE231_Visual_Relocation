...
class Distillation(BaseDataset):
    ...
    def _init_dataset(self, **config):
        ...
        if config['load_targets']:
            for i, target in enumerate(config['targets']):
                for im in config['image_dirs']:
                    # print(im)
                    # print(target['dir'])
                    # print(Path(DATA_PATH, im).parent.parent)
                    assert Path(Path(DATA_PATH, im).parent.parent, target['dir']).exists()
                data[i] = []
        ...
        for i, image_dir in enumerate(config['image_dirs']):
            if image_dir == "google_landmarks/images":
        ...
        if config['load_targets']:
            ...
            for im, n in zip(im_paths, names):
                ...
                for target in config['targets']:
                    # target_path = Path(Path(im).parent.parent, target['dir'], f'{n}.npz')
